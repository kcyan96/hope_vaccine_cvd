...
cohort[, postacute.28 := index.date+28]
excl <- cohort[(cohort$`Date of vaccination.1st`>=index.date&cohort$`Date of vaccination.1st`<postacute.28) | 
                 (cohort$`Date of vaccination.2nd`>=index.date&cohort$`Date of vaccination.2nd`<postacute.28) | 
                 (cohort$`Date of vaccination.3rd`>=index.date&cohort$`Date of vaccination.3rd`<postacute.28) |
                 (cohort$`Date of vaccination.4th`>=index.date&cohort$`Date of vaccination.4th`<postacute.28) | 
                 (cohort$`Date of vaccination.5th`>=index.date&cohort$`Date of vaccination.5th`<postacute.28)][,.(match)]
...
sink(paste0("output/T2_acute_add_dose_log_jiayi.txt"), append=F, split=T)
print("Table 2")
for(oc in OUTCOMES) {
  cat(oc, "\n")
