...
res_a <- sapply(c("postacute"), function(p) run_fit(cohort, p, OUTCOMES), simplify=F, USE.NAMES=T)
res_a_IR <- sapply(c("postacute"), function(p) run_fit_IR(cohort, p, OUTCOMES), simplify=F, USE.NAMES=T)
...
res_p <- sapply(c("acute"), function(p) run_fit(cohort, p, OUTCOMES), simplify=F, USE.NAMES=T)
res_p_IR <- sapply(c("acute"), function(p) run_fit_IR(cohort, p, OUTCOMES), simplify=F, USE.NAMES=T)
sink(paste0("LONGCOVID_CC/output/COMP_", COMP, PHASE, "_log.txt"), append=F, split=T)
print(COMP)
res_p
print("IR")
