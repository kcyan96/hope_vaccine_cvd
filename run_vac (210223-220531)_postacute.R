...
# 1. Load cohort ----
cohort <- readRDS("LONGCOVID_CC/longcovid.cohort.matched_postacute_210223.220531.RDS") # n=4479513
# 
...
cohort <- cohort[is.na(death_date_ymd) | death_date_ymd >= index.date]   # n=4479513
...
cohort_full <- copy(cohort[postacute==1])

saveRDS(cohort_full, "LONGCOVID_CC/cohort_postacute.RDS")
...
## Weighting ---------------------------------------------
cohort_full <- readRDS("LONGCOVID_CC/cohort_postacute.RDS")
# SELECT comparison
...
library(WeightIt)
W.glm.postacute <- weightit(as.formula(PS_formula("group",4)), data=cohort, estimand="ATE", method="ps")
PS_assess(W.glm.postacute)
cohort[, wts_postacute := W.glm.postacute$weights]
...
save(W.glm.postacute, tb1, tb1.iptw, file=paste0("LONGCOVID_CC/COMP_", COMP, PHASE, "_W.glm.RData"))
...
rm(cohort, tb1, tb1.iptw, W.glm, W.glm.postacute, xl_tabs)
gc()
...
cohort_180 <- readRDS("LONGCOVID_CC/cohort_postacute_180.RDS")
# SELECT comparison
...
cohort_pcr <- readRDS("LONGCOVID_CC/cohort_postacute_pcr.RDS")
# SELECT comparison
